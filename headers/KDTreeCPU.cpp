...
KDTreeCPU::KDTreeCPU( int num_tris, vec3 *tris, int num_verts, vec3 *verts )
{
	...
	Volume bbox = computeTightFittingBoundingBox( num_verts, verts );
...
SplitAxis KDTreeCPU::getLongestBoundingBoxSide( vec3 min, vec3 max )
{
...
{
	vec3 tri = tris[tri_index];
	vec3 v0 = verts[( int )tri[0]];
	vec3 v1 = verts[( int )tri[1]];
	vec3 v2 = verts[( int )tri[2]];
...
vec3* KDTreeCPU::getMeshVerts( void ) const
{
	return verts;
}

vec3* KDTreeCPU::getMeshTris( void ) const
{
	return tris;
}
...
Volume KDTreeCPU::computeTightFittingBoundingBox( int num_verts, vec3 *verts )
{
...
Volume KDTreeCPU::computeTightFittingBoundingBox( int num_tris, int *tri_indices )
{
...
////////////////////////////////////////////////////
KDTreeNode* KDTreeCPU::constructTreeMedianSpaceSplit( int num_tris, int *tri_indices, Volume bounds, int curr_depth )
{
	// Create new node.
	...
	Volume left_bbox = bounds;
	Volume right_bbox = bounds;
	if ( longest_side == X_AXIS ) {
		median_val = bounds.min.x() + ( ( bounds.max.x() - bounds.min.x() ) / 2.0f );
		left_bbox.max.setX(median_val);
		right_bbox.min.setX(median_val);
	}
	else if ( longest_side == Y_AXIS ) {
		median_val = bounds.min.y() + ( ( bounds.max.y() - bounds.min.y() ) / 2.0f );
		left_bbox.max.setY(median_val);
		right_bbox.min.setY(median_val);
	}
	else {
		median_val = bounds.min.z() + ( ( bounds.max.z() - bounds.min.z() ) / 2.0f );
		left_bbox.max.setZ(median_val);
		right_bbox.min.setZ(median_val);
	}
...
bool KDTreeCPU::intersect(hitInfo &hitinfo, vec3 &hit_point) const
{
	hitinfo.t = INFINITY;
	bool hit = intersect(root, hitinfo);
	if ( hit ) {
		hit_point = hitinfo.origin + ( hitinfo.t * hitinfo.ray );
	}
	return hit;
}

// Private recursive call.
bool KDTreeCPU::intersect( KDTreeNode *curr_node, hitInfo &hitinfo) const
{
	// Perform ray/AABB intersection test.
	float t_near, t_far;
	bool intersects_aabb = Intersections::aabbIntersect( curr_node->bbox, hitinfo.origin, hitinfo.ray, t_near, t_far );
	...
	if ( intersects_aabb ) {
		...
		if ( !curr_node->left && !curr_node->right ) {
			...
			for ( int i = 0; i < curr_node->num_tris; ++i ) {
				//cerr << curr_node->num_tris << endl;
				vec3 tri = tris[curr_node->tri_indices[i]];
				vec3 v0 = verts[( int )tri[0]];
				vec3 v1 = verts[( int )tri[1]];
				vec3 v2 = verts[( int )tri[2]];

				// Perform ray/triangle intersection test.
				hitInfo triCheck = hitinfo;
				Triangle triangle;
				triangle.p[0] = v0;
				triangle.p[1] = v1;
				triangle.p[2] = v2;
				bool intersects_tri = Intersections::triHit(triangle, triCheck);

				if ( intersects_tri ) {
					intersection_detected = true;
					if ( triCheck.t < hitinfo.t ) {
						hitinfo = triCheck;
					}
				}
			}
		...
		// Else, recurse.
			...
			if ( curr_node->left ) {
				hit_left = intersect( curr_node->left, hitinfo);
			}
			if ( curr_node->right ) {
				hit_right = intersect( curr_node->right, hitinfo);
			}
...
// Public-facing wrapper method.
bool KDTreeCPU::singleRayStacklessIntersect( const vec3 &ray_o, const vec3 &ray_dir, float &t, vec3 &hit_point) const
{
	// Perform ray/AABB intersection test.
	float t_near, t_far;
	...
	if ( intersects_root_node_bounding_box ) {
		bool hit = singleRayStacklessIntersect( root, ray_o, ray_dir, t_near, t_far);
		if ( hit ) {
			t = t_far;
...
bool KDTreeCPU::singleRayStacklessIntersect( KDTreeNode *curr_node, const vec3 &ray_o, const vec3 &ray_dir, float &t_entry, float &t_exit) const
{
	bool intersection_detected = false;
	...
	while ( t_entry < t_exit && t_entry > t_entry_prev ) {
		...
		vec3 p_entry = ray_o + ( t_entry * ray_dir );
		...
		for ( int i = 0; i < curr_node->num_tris; ++i ) {
			...
			triangle.p[1] = v1;
			triangle.p[2] = v2;
			bool intersects_tri = Intersections::triHit(triangle, hitinfo);
		...
		vec3 p_exit = ray_o + ( t_entry * ray_dir );
...
////////////////////////////////////////////////////
void KDTreeCPU::optimizeRopes( KDTreeNode *ropes[], Volume bbox )
{
